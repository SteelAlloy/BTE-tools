...
export default function xmlToJson(xml) {
	...
	// Get tag name
	var first = getFirst(copy)

	// Get first tag with attributes
	var remove = getFirstTag(copy)
...
function getChildren(copy2) {
	...
	while (copy2.startsWith('<')) {
		var first = getFirst(copy2)
		var firstTag = getFirstTag(copy2)
		...
		// Get first value
		var firstValue = copy2.slice(0, lastIndex)
		// Remove first value
